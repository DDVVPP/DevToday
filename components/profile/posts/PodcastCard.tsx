  ...
  return (
    ...
    >
      <Link
        ...
        className="group/contentcard relative flex h-[260px] break-inside-avoid-column flex-col justify-between rounded-[16px] bg-white-100 p-5 transition hover:bg-[#e1e2e6] hover:duration-300 dark:bg-dark-800 dark:hover:bg-dark-700"
      >
          ...
          <div className="mt-1 flex h-10 flex-wrap gap-x-2.5 overflow-hidden">
            {podcast?.tags &&
              podcast.tags.map((tag, idx) => (
                <ContentTags key={idx} tag={tag} />
              ))}
          </div>
        </div>

        <Link href={`/profile/${user.id}`} className="w-fit justify-self-end">
          <UserSection
            image={user.image!}
